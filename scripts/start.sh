...
if [ "$OS_TYPE" = "Darwin" ]; then
  # macOS: use the cwd basename as WINDOW_MATCH.
  # Terminal.app window titles include the cwd, and the AppleScript matcher
  # uses "contains", so the basename is a stable, unique-enough substring.
  # Using the full window name would be fragile (it includes process name,
  # dimensions, etc. which change dynamically).
  CWD_BASE="$(basename "$PWD")"
  if [ -n "$CWD_BASE" ] && [ "$CWD_BASE" != "/" ]; then
    DETECTED_MATCH="$CWD_BASE"
    DETECTED_NAME="$CWD_BASE"
  fi
